...
class TestCaseEditor(QMainWindow):
    ...
    def create_form_widget(self) -> QWidget:
        ...
        # Created At (компактный, только для чтения)
        created_container = QVBoxLayout()
        created_container.setSpacing(5)
        created_label = QLabel("Создан:")
        created_label.setFont(QFont("Segoe UI", 9, QFont.Bold))
        created_label.setStyleSheet("color: #8B9099;")
        created_container.addWidget(created_label)
        
        self.created_at_input = QLineEdit()
        self.created_at_input.setReadOnly(True)
        self.created_at_input.setPlaceholderText("Дата создания")
        self.created_at_input.setMinimumHeight(32)
        self.created_at_input.setMaximumWidth(180)
        ...
        """)
        created_container.addWidget(self.created_at_input)
        row1.addLayout(created_container)
        
        row1.addSpacing(15)
        
        # Updated At (компактный, только для чтения)
        updated_container = QVBoxLayout()
        updated_container.setSpacing(5)
        updated_label = QLabel("Обновлён:")
        updated_label.setFont(QFont("Segoe UI", 9, QFont.Bold))
        updated_label.setStyleSheet("color: #8B9099;")
        updated_container.addWidget(updated_label)
        
        self.updated_at_input = QLineEdit()
        self.updated_at_input.setReadOnly(True)
        self.updated_at_input.setPlaceholderText("Дата обновления")
        self.updated_at_input.setMinimumHeight(32)
        self.updated_at_input.setMaximumWidth(180)
        ...
        # Folder ID (компактный)
        folder_label = QLabel("Folder ID:")
        folder_label.setFont(QFont("Segoe UI", 9, QFont.Bold))
        folder_label.setStyleSheet("color: #8B9099;")
        details_layout.addWidget(folder_label)
        
        self.folder_id_input = QLineEdit()
        self.folder_id_input.setPlaceholderText("ID папки/категории")
        self.folder_id_input.setMinimumHeight(28)
        self.folder_id_input.textChanged.connect(self.mark_as_changed)
        details_layout.addWidget(self.folder_id_input, 1)
        ...
        self.description_input.textChanged.connect(self.mark_as_changed)
        ...
        # Сворачиваемая секция: Labels (метки)
        labels_box = CollapsibleBox("▼ Метки (Labels)")
        ...
        self.labels_table.setHorizontalHeaderLabels(["Тип (epic/feature/story)", "Значение"])
        self.labels_table.horizontalHeader().setSectionResizeMode(0, QHeaderView.Fixed)
        self.labels_table.horizontalHeader().setSectionResizeMode(1, QHeaderView.Stretch)
        self.labels_table.setColumnWidth(0, 200)
        self.labels_table.verticalHeader().setVisible(True)
        self.labels_table.setMinimumHeight(150)
        self.labels_table.setMaximumHeight(200)
        self.labels_table.setAlternatingRowColors(True)
        self.labels_table.itemChanged.connect(self.mark_as_changed)
        
        # Настройка внешнего вида таблицы меток
        ...
        btn_add_label = QPushButton("➕ Добавить метку")
        btn_add_label.clicked.connect(self.add_label_row)
        btn_add_label.setMinimumHeight(35)
        labels_buttons_layout.addWidget(btn_add_label)
        
        btn_remove_label = QPushButton("➖ Удалить выбранную")
        btn_remove_label.clicked.connect(self.remove_label_row)
        btn_remove_label.setMinimumHeight(35)
        labels_buttons_layout.addWidget(btn_remove_label)
        
        labels_buttons_layout.addStretch()
        
        labels_layout.addLayout(labels_buttons_layout)
        
        labels_box.set_content_layout(labels_layout)
    ...
    def add_label_row(self, name: str = "", value: str = ""):
        """Добавление строки в таблицу меток"""
        row_position = self.labels_table.rowCount()
        self.labels_table.insertRow(row_position)
        ...
        self.labels_table.setItem(row_position, 0, name_item)
        self.labels_table.setItem(row_position, 1, value_item)
        
        # Устанавливаем высоту строки
        self.labels_table.setRowHeight(row_position, 50)
    
    def remove_label_row(self):
        """Удаление выбранной строки из таблицы меток"""
    ...
    def load_test_case_to_form(self, test_case: Dict):
        ...
        self.use_case_id_input.setText(test_case.get('use_case_id', ''))
        self.folder_id_input.setText(test_case.get('folder_id', ''))
        ...
        # Описание
        self.description_input.setText(test_case.get('description', ''))
        ...
        for action in test_case.get('steps', []):
            self.add_action_row(
                action.get('step', ''),
                action.get('expected_res', '')
            )
        
        # Очистка и загрузка меток
    ...
    def get_form_data(self) -> Dict:
        ...
        # Сбор меток из таблицы
        labels = []
        for row in range(self.labels_table.rowCount()):
            name_item = self.labels_table.item(row, 0)
            value_item = self.labels_table.item(row, 1)
            
            name_text = name_item.text() if name_item else ""
            value_text = value_item.text() if value_item else ""
            
            if name_text or value_text:  # Добавляем только непустые метки
                labels.append({
                    'name': name_text,
                    'value': value_text
                })
        ...
        return {
            ...
            'use_case_id': self.use_case_id_input.text(),
