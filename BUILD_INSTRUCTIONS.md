...
**Примечание:** Файл `settings.json` не включается в сборку, так как настройки сохраняются в стандартной директории macOS (см. раздел "Сохранение настроек" ниже).

## Сохранение настроек

В собранном приложении настройки сохраняются в стандартной директории macOS:
```
~/Library/Application Support/Test Case Editor/settings.json
```

При первом запуске приложение автоматически создаст эту директорию и файл настроек. Если у вас есть старый файл `settings.json` рядом с `.app` bundle, он будет автоматически скопирован в новое место при первом запуске.

В режиме разработки настройки сохраняются в корне проекта в файле `settings.json`.
